   ...
   if (is_uploaded_file($foto['tmp_name'])) {
      ...
      if (!in_array(($tipe),$ext)){ //cek ekstensi file
         ...
         if (move_uploaded_file($foto['tmp_name'],$dir.$newName)) {
            ...
            $sql = $con->prepare("UPDATE t_calon_rw SET nama_calon = ? , foto = ?, visi = ?, misi = ? WHERE id_rw = ?") or die($con->error);
            $sql->bind_param('sssss', $nama, $newName, $visi, $misi, $id);
            $sql->execute();

            header('location:../dashboard.php?page=rw');
      ...
      $sql = $con->prepare("UPDATE t_calon_rw SET nama_calon = ?, visi = ?, misi = ? WHERE id_rw = ?") or die($con->error);
